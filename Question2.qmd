...
leaflet(data = map_production_sf) %>%
  addProviderTiles(providers$CartoDB.Positron) %>%  # Base map
  addPolygons(
    fillColor = ~colorNumeric("YlGn", Total_Value)(Total_Value),  # Color based on production value
    weight = 1,
    color = "black",
    fillOpacity = 0.7,
    label = ~paste(Area, "<br>Production:", Total_Value, "tons"),
    highlightOptions = highlightOptions(weight = 2, color = "blue", bringToFront = TRUE)
  ) %>%
  addLegend(
    pal = colorNumeric("YlGn", map_production$Total_Value),
    values = ~Total_Value,
    title = "Livestock Production (tons)",
