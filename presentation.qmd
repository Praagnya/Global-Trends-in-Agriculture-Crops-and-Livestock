...
livestock_data_filtered <- livestock_data %>%
  filter(Item %in% livestock_items,
         Element %in% c("Production", "Area harvested", "Stocks")) %>%
  select(Area, Item, Element, starts_with("Y"))
...
ggplot() +
  geom_sf(data = world, fill = "gray90", color = "white") + # Add world map base layer
  geom_sf(data = map_production, aes(geometry = geometry, fill = cut(Total_Value, 
    breaks = c(0, 1e5, 5e5, 1e6, 5e6, 1e7, Inf), 
    labels = c("<100K", "100K-500K", "500K-1M", "1M-5M", "5M-10M", ">10M"))), 
    color = "black") +
  scale_fill_manual(
    values = c("khaki1", "gold", "orange", "darkorange", "pink", "red"), # Custom colors
    name = "Production (tons)" # Legend title
  ) +
  labs(
  ...
  theme(
    ...
    legend.title = element_text(size = 12, face = "bold", hjust = 0.5), # Center-align legend title
